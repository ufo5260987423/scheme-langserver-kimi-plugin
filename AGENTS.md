...
5. **崩溃时引导用户提交调试报告**：当 scheme-langserver 崩溃且 Bridge 已自动生成调试报告时，Kimi 应主动告知用户报告位置，并建议用户审查后提交到 https://github.com/ufo5260987423/scheme-langserver/issues 。这是改善 Kimi 长期使用体验的关键闭环。
6. **不盲从**：如果 LSP 说某个代码有错误，但 Kimi 从语义上判断是正确的（例如使用了实现特定扩展），Kimi 应优先相信自己的判断
