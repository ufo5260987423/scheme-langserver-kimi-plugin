...
| `workspace/symbol` | 获取工作区中所有匹配的符号（**需 scheme-langserver ≥ 2.1.0 (tested up to 2.1.1)**） | Kimi 需要在整个项目中搜索某个函数或变量时，快速定位 |
...
- **工具可用性受服务器版本限制**：
  - `workspace/symbol` 需要 **scheme-langserver ≥ 2.1.0 (tested up to 2.1.1)**（此前版本不支持）
...
2. **置信度分级**：
   ...
   - **中置信度**：`completion`（补全列表可能漏掉宏生成的标识符）；`workspace/symbol`（≥ 2.1.0 / tested up to 2.1.1，跨文件符号搜索准确度取决于索引完整性）
